...
Copy each block into the matching field. Full detail lives in `../deliverables/Project Brief/Project Brief.md` and `../deliverables/AI Usage Statement/AI_Usage_Statement_Technical.md`.
...
Refusal is a first-class outcome. An escalation carries the transcript, the evidence and the exact action Axel was about to take, which is what makes an automated decision reviewable rather than merely fast. An agent that always acts is not trustworthy, it is just fast.

And the work stays inside the customer's process. Cluster writes raise a standard change record with a rollback plan; a command outside the typed action set requires a named human approver holding a capability deliberately withheld from the analysts who issue routine actions.

We make no performance, savings or accuracy claim. Nothing in the repository supports one, so we do not make one.

### Roadmap — *What's next for this project?*

**Next:** close the known gaps. Sign the Windows binary, so SmartScreen and managed-device policy stop blocking the installer. Publish images and a release pipeline — the Dockerfiles and chart exist, but images are built locally today. Make lexical-only retrieval loud rather than silent when no embedding key is configured. And exercise the human-approved device command path end to end against a live gateway; its coverage is unit-level today.

**Then:** reach. A pixel-based fallback for GUI surfaces that expose no accessibility tree — canvas apps, remote desktop, Citrix — which is blocked on a finding rather than on effort. Connectors beyond Kubernetes: cloud consoles, virtual machines, databases. Seeding the CMDB from an existing source of truth, so the first ticket about a service is not the first thing the platform knows about it. And co-deploying inference inside the customer's perimeter, so ticket text never leaves their network.
...
**Start.** Repo Wiki and codebase indexing carried orientation across a workspace that is deliberately not a monorepo — six projects, four toolchains, no shared build graph. Agent mode did the bulk of implementation: the oRPC contracts and handlers, the Drizzle schema across 144 tables, the Go daemon and its PowerShell action scripts, both frontends, the Helm templates.

**Middle.** Quest in Experts Mode carried the vertical slices that cross every layer at once — a single device action is a schema in the API, a tool parameter in the Python agent, an enum in the proto, a dispatch case and a script in the Go binary, and a screen in the dashboard. Quest's spec-driven mode carried the two places where acceptance criteria had to exist before code: the human-approved device command path, and the change-gated cluster write. Workspace rules held the invariants as always-on context and kept Git read-only, so no commit happened without us asking for it. Two MCP servers helped throughout — Context7 for current library docs across fast-moving dependencies, Chrome DevTools for checking rendered behaviour in a real browser instead of inferring it from the component tree.

**End.** The integrated terminal ran every project's own gates after each change, and checkpoints let us roll back generated work that failed one rather than patching over it — both of our failed Helm migration strategies went out that way.

**The experience, honestly.** It was fastest where the work was wide and shaped: schema, contracts, plumbing, UI, and propagating one change through five files that must agree. It was weakest where correctness depended on the real machine, and that is where we spent our own time. A PowerShell facet Qoder generated measured 50 seconds on a real six-printer laptop and had to be rewritten against CIM. A value bound through `powershell.exe -Command` was silently re-split on spaces, which only running it revealed. A device action we designed and built was then deleted, because no non-admin, edition-portable read could observe its effect — `gpresult /x` does not exist on Windows Home, which we found by running it.
