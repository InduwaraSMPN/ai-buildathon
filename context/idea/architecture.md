...
**Deny by default is structural.** `os` and `authenticatedProcedure` are deliberately not exported from `server/orpc.ts`, so a procedure cannot be written without naming a capability, and a test asserts it. Do not export them for convenience. `appRouter` names its members explicitly rather than spreading the sub-routers, which is deliberate — a procedure has to be mounted on purpose — and a second test asserts the other direction, that every declared contract procedure appears there. Without it a procedure can have a contract, a handler, and a passing typecheck while being unreachable, and the first sign is a 404.

**A cookie-bearing write must prove it is same-site.** Session cookies are issued `SameSite=None`, because the frontends are served from a different origin than the API. `hono/cors` cannot help — it only sets response headers and never refuses a request — and a `multipart/form-data` POST is CORS-safelisted, so it triggers no preflight. A middleware ahead of the handlers rejects any non-idempotent request that carries a cookie and declares a cross-site origin. Requests authenticating with a bearer token carry no ambient credential and are deliberately exempt.

**An API key can never exceed its owner.** Capabilities are snapshotted onto the key at issue time for auditability, and intersected with what the owning account holds at request time for authority. Revoking a role therefore revokes every key that rode on it, rather than leaving one valid until its TTL expires.
...
**Email threads by reference, and by sender.** Matching is on retained ticket-reference tokens with a word boundary — subject similarity is not threading. A token alone is not authority either: ticket references are sequential and appear in every notification the system sends, so threading additionally requires the sender address to resolve to that ticket's reporter, and a message that does not match starts its own ticket rather than joining someone else's. The comment is attributed to the resolved sender, never to the ticket's reporter.
...
- **Ownership is a second, separate act.** Enrolment binds a machine to the gateway and issues its credential; it cannot say who is sitting at it, because a laptop dialling out has nobody to ask. So `devices.owner_id` stays null until an employee claims the machine, and every owner-scoped path — the employee's own device list, the intake composer's picker, the ticket-to-device binding the whole device tier rests on — reads that column. The gateway issues a short claim code with the credential, the daemon persists it, `axel-cli status` prints it, and the employee types it into the portal. It is stored hashed like every other bearer value here, expires in a day, and is cleared on use, so a screenshot of it in a chat thread is worth nothing afterwards. Self-service is the point: IT never types an employee's name against a serial number.
...
- **Replay.** On reconnect the device reports the last sequence it processed and the gateway replays past it. Deliberately a bounded in-memory outbox, not durable queuing. The sequence is written to disk at the moment a command is *dequeued for execution*, not when it is accepted into the local queue — the gateway replays only past what the device reported, so anything still queued when the stream drops has to remain replayable. Writing it on accept consumed the sequence for commands that never ran, and the gateway then had nothing left to replay. The gateway keeps its outbox across an ordinary disconnect for the same reason and discards it only on revocation.
...
**Both channels on that port authenticate, because both are reachable from an employee laptop.** The gateway requires TLS material and will not start without it. A device proves itself with a hashed per-device credential on hello and is refused once revoked; the daemon dials with real transport credentials. A worker proves itself with a shared secret the operator sets on the API and on the agent, compared in constant time — without which the gateway refuses the connection rather than registering an unknown worker. That second half is not decoration: a worker id alone is not an identity, and the channel it opens carries ticket text, reporter details, and the right to call every tool including the device ones. It also fails closed, so an operator who never sets the secret gets no agent channel rather than an open one.
...
**Which namespaces it may reach is an allowlist the API enforces itself.** The namespace on a cluster tool call is model-supplied, and the chart's per-namespace Role binds only the in-cluster ServiceAccount — under `kubeconfig` mode the pod authenticates with a mounted credential instead and that Role is inert. So the allowlist is applied where the call is made rather than left to RBAC alone, and it stays correct under both connection modes. Skipping TLS verification is likewise a process-level opt-in rather than a field on a stored credential: an environment row asking for it is refused and logged, so nobody able to write that table can quietly downgrade the API's connection to a customer cluster.
...
144 tables across 46 schema modules, Better Auth's own included. Grouped by what they carry:
...
| Devices | `devices` with their enrolment and claim state, enrolment tokens, `device_commands`, command proposals |
...
pgvector is used. `search_documents` carries a `vector(1536)` embedding alongside a weighted full-text index, and retrieval fuses the two rank lists by reciprocal rank. The HNSW cosine indexes are partial, one per retrieval scope, because every vector query here carries mandatory non-vector predicates: a single index over the whole table is either ignored by the planner or post-filtered, and post-filtering hands back the nearest neighbours of the *table* rather than of the scope — which for the portal's deflection means the agent-run projections crowd out the knowledge articles it was asking for. The JSONB metadata those scopes filter on is indexed too. Embeddings are written only when an embedding key is configured; without one the vector leg is skipped, retrieval reports `mode: "lexical"`, and the run proceeds. Degrading rather than failing is deliberate. Degrading *silently* is not, and it is unfixed.
...
- **Duplicate suppression exists; durability does not.** A repeated tool call within a run returns its recorded result, and a device rejects a sequence it has already accepted — but that rejection reports the outcome as indeterminate rather than replaying it, and the gateway's outbox is in memory, so a command in flight when the API restarts is lost. Device delivery is at-most-once by choice: the sequence is persisted before the command runs, so a laptop that loses power part-way through drops that command rather than repeating it, and neither side then knows whether it took effect. Retrying safely needs durable results and idempotency keys, which are not built. Widening the action set widens the consequence of all of it.
- **Both gRPC channels are authenticated; the binary is unsigned.** The gateway requires TLS, verifies a hashed per-device bearer credential before registration or replay, supports single-use enrolment, rotation, and revocation, and lets clients add a customer CA without disabling verification. The agent channel on the same port requires a shared secret and refuses every connection without one. What that authentication does not carry is mutual TLS: the gateway presents a certificate and does not ask for one, so the credential is the whole of the client's identity. Authenticode signing of the binary remains deferred pending certificate procurement.
