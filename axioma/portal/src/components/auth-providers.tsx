...
/**
 * The identity providers this portal expects to federate with, shown whether
...
/**
 * The "Or continue with" row on the portal's auth card.
 *
 * Every provider is rendered every time. One an administrator has enabled is
 * live and starts an OIDC redirect; one nobody has configured is disabled and
 * says so on hover, which is the difference between "this portal cannot do
 * that" and "your organisation has not switched it on".
 */
export function AuthProviders({ callbackURL }: { callbackURL: string }) {
			...
			<Field className="grid grid-cols-3 gap-4">
				{known.map(({ label, icon: Icon, match }) => (
					<Button
						...
						title={match ? match.name : ssoCopy.notConfigured(label)}
					...
					>
						...
						<span className="sr-only">
							{match
								? ssoCopy.signIn(match.name)
								: ssoCopy.notConfigured(label)}
						</span>
					</Button>
				))}
				...
				{extra.map((provider) => (
					...
					>
						...
						<span className="truncate">{ssoCopy.signIn(provider.name)}</span>
					</Button>
				))}
			</Field>
		</>
