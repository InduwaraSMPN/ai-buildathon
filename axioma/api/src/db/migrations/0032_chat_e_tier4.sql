...
WHERE "ticket_origin" NOT IN (SELECT "key" FROM "ticket_origins");
UPDATE "messaging_threads" SET "origin_key" = 'chat'
WHERE "origin_key" NOT IN (SELECT "key" FROM "ticket_origins");
UPDATE "ticket_mail_origins" SET "ticket_origin" = 'email'
WHERE "ticket_origin" NOT IN (SELECT "key" FROM "ticket_origins");

ALTER TABLE "mailboxes"
  ADD CONSTRAINT "mailboxes_ticket_origin_fk"
  FOREIGN KEY ("ticket_origin") REFERENCES "ticket_origins"("key") ON DELETE restrict;
ALTER TABLE "messaging_threads"
  ADD CONSTRAINT "messaging_threads_origin_key_fk"
  FOREIGN KEY ("origin_key") REFERENCES "ticket_origins"("key") ON DELETE restrict;
