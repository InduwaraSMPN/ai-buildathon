...
export class FixedWindowRateLimiter {
	...
	#globalBucket: Bucket = { window: -1, count: 0 };
	readonly #keyBuckets = new Map<string, Bucket>();

	constructor(options: { perKey: number; global: number; windowMs: number }) {
		for (const [name, value] of Object.entries(options)) {
			if (!Number.isSafeInteger(value) || value <= 0) {
				throw new Error(`${name} must be a positive integer`);
			}
		}
		this.#perKey = options.perKey;
		this.#global = options.global;
		this.#windowMs = options.windowMs;
	}

	consume(keyId: string, now = Date.now()): RateLimitResult {
		if (!keyId || !Number.isFinite(now))
			throw new Error("Invalid rate-limit input");
		const window = Math.floor(now / this.#windowMs);
		if (this.#globalBucket.window !== window) {
			this.#globalBucket = { window, count: 0 };
			this.#keyBuckets.clear();
		}
		const keyBucket = this.#keyBuckets.get(keyId) ?? { window, count: 0 };
		const resetAtMs = (window + 1) * this.#windowMs;
		if (
			this.#globalBucket.count >= this.#global ||
			keyBucket.count >= this.#perKey
		) {
			return {
				allowed: false,
				remaining: 0,
				resetAt: new Date(resetAtMs),
				retryAfterMs: Math.max(1, resetAtMs - now),
			};
		}

		this.#globalBucket.count += 1;
		keyBucket.count += 1;
		this.#keyBuckets.set(keyId, keyBucket);
		return {
			allowed: true,
			remaining: Math.min(
				this.#global - this.#globalBucket.count,
				this.#perKey - keyBucket.count,
			),
			resetAt: new Date(resetAtMs),
		};
	}
