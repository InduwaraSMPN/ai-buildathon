...
const ticketSelection = {
	...
	statusColour: sql<string | null>`(select colour from ${ticketStatuses} where key = ${tickets.status})`,
	route: tickets.route,
...
async function findTicketMessages(ticketId: string) {
	return db
		.select()
		.from(ticketMessages)
...
export const appRouter = {
		...
		async ({ context, input }) => {
			if (input.recordType === "service_request")
				throw new ORPCError("BAD_REQUEST", {
					message: "Service requests must be submitted through the request catalogue",
				});
	...
	addMyTicketMessage: capabilityProcedure(
		...
		await db
			...
			.where(and(eq(tickets.id, input.ticketId), eq(tickets.status, "pending")));
	...
	startRun: capabilityProcedure("run.start").startRun.handler(({ input }) =>
