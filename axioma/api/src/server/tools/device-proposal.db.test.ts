 ...
 *
 * These run against the development database, which is also the database the
 * demo is shown from, so every row is seeded through `withFixtures` and removed
 * again from its `finally`. Anything left behind here becomes demo data.
 */
...
test("a device that has not opted in cannot even be proposed against", async () => {
	await withFixtures(async (fixtures) => {
		const deviceId = await fixtures.device({ executionEnabled: false });
...
test("proposing writes a pending row and nothing else", async () => {
	await withFixtures(async (fixtures) => {
		const deviceId = await fixtures.device({ executionEnabled: true });
		const ticketId = await fixtures.ticket();
...
test("a device proposal never lands in the catalogue approvals queue", async () => {
	await withFixtures(async (fixtures) => {
		...
		const count = await approvalCount(ticketId);
...
test("a dispatched command records the approval that authorised it", async () => {
	await withFixtures(async (fixtures) => {
		const deviceId = await fixtures.device({ executionEnabled: true });
		const proposalId = fixtures.id("proposal");
...
test("a proposal records who set its run going", async () => {
	await withFixtures(async (fixtures) => {
		const deviceId = await fixtures.device({ executionEnabled: true });
		const ticketId = await fixtures.ticket();
		const starterId = await fixtures.user({ name: "Run starter" });
		const runId = await fixtures.run({ ticketId, startedById: starterId });
		...
		const autoRunId = await fixtures.run({ ticketId });
...
/**
 * `approvals` has no Drizzle table in this file's import set, and the count is
 * all these tests need from it.
 */
async function approvalCount(ticketId: string) {
